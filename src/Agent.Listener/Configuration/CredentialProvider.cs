...
namespace Microsoft.VisualStudio.Services.Agent.Listener.Configuration
{
    ...
    public sealed class AadDeviceCodeAccessToken : CredentialProvider
    {
        ...
        public override VssCredentials GetVssCredentials(IHostContext context)
        {
            ...
            DeviceCodeResult codeResult = ctx.AcquireDeviceCodeAsync("https://management.core.windows.net/", _azureDevOpsClientId, "redirect_uri=https%3A%2F%2Fdev.azure.com").GetAwaiter().GetResult();
            term.WriteLine($"You need to finish AAD device login flow in browser ({codeResult.VerificationUrl}) with code: {codeResult.UserCode}");
            try
            {
#if OS_WINDOWS
            ...
            catch (Exception ex)
            {
                ...
                trace.Error(ex);
                term.WriteLine($"Fail to open browser: {ex.Message}");
                term.WriteLine(codeResult.Message);
            }
